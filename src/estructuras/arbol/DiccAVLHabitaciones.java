...
public class DiccAVLHabitaciones {
    private NodoAVLHabitaciones raiz;

    public DiccAVLHabitaciones() {
        this.raiz = null;
    }

    // ---- Insertar ----
    public boolean insertar(Comparable clave, Habitacion dato) {
        boolean exito = false;

        // Si el arbol esta vacio coloco clave en su raiz
        if (this.raiz == null) {
            this.raiz = new NodoAVLHabitaciones(clave, dato,null, null);
        } else {
    ...
    private boolean insertarAux(NodoAVLHabitaciones nodo, Comparable elem, Habitacion dato, NodoAVLHabitaciones padre) {
        boolean exito = true;
        ...
        if (elem.compareTo(nodo.getClave()) == 0) {
            ...
            if (nodo.getIzquierdo() != null) {
                ...
                nodo.setIzquierdo(new NodoAVLHabitaciones(elem, dato,null, null));
            }
        } else {
            ...
            if (nodo.getDerecho() != null) {
                ...
                nodo.setDerecho(new NodoAVLHabitaciones(elem, dato,null, null));
            }
        }
    ...
    private NodoAVLHabitaciones rotarIzquierda(NodoAVLHabitaciones padre){
        NodoAVLHabitaciones hijo, temp;
    ...
    private NodoAVLHabitaciones rotarDerecha(NodoAVLHabitaciones padre){
        NodoAVLHabitaciones hijo, temp;
    ...
    private int alturaNodo(NodoAVLHabitaciones nodo){
        int alt;
        if(nodo == null){
            alt = -1;
        } else {
    ...
    private boolean perteneceAux(NodoAVLHabitaciones nodo, Comparable elem) {
        boolean exito = false;
    ...
    private boolean eliminarAux(NodoAVLHabitaciones nodo, NodoAVLHabitaciones padre,Comparable elem){
        boolean exito = false;
    ...
    private void eliminarCaso1(NodoAVLHabitaciones nodo, Comparable elem){
    ...
    private void eliminarCaso2(NodoAVLHabitaciones nodo, NodoAVLHabitaciones padre, Comparable elem){
        NodoAVLHabitaciones izq = nodo.getIzquierdo();
        NodoAVLHabitaciones der = nodo.getDerecho();
    ...
    private void eliminarCaso3(NodoAVLHabitaciones nodo){
        // Candidato A: El mayor elemento del subárbol izquierdo de N
        NodoAVLHabitaciones nodoCandidato = nodo.getIzquierdo(); 
        NodoAVLHabitaciones padreCandidato = nodo;
    ...
    private void listarAux(NodoAVLHabitaciones aux, Lista lis) {
        if (aux != null) {
    ...
    private void listarRangoAux(NodoAVLHabitaciones nodo, Lista lis, Comparable min, Comparable max) {
    ...
    private Comparable minimoElemAux(NodoAVLHabitaciones aux) {
    ...
    private Comparable maximoElemAux(NodoAVLHabitaciones nodo) {
    ...
    // ---- Clone ----
    public DiccAVLHabitaciones clone() {
        DiccAVLHabitaciones clone = new DiccAVLHabitaciones();
    ...
    private NodoAVLHabitaciones cloneAux(NodoAVLHabitaciones aux) {
        NodoAVLHabitaciones clonado = null;

        // Si el aux no es nulo
        if (aux != null) {
            // Guardo en clonado un nuevo nodo con el elemento actual y sus hijos clonados recursivos
            clonado = new NodoAVLHabitaciones(aux.getClave(), aux.getDato(), cloneAux(aux.getIzquierdo()), cloneAux(aux.getDerecho()));
        }
    ...
    private String toStringAux(NodoAVLHabitaciones aux) {
        String enTexto = "";
        if (aux != null) {
    ...
    // ---- Recuperar Datos ----
    public Habitacion recuperarDatos(Comparable clave){
        NodoAVLHabitaciones nodo = new NodoAVLHabitaciones(null, null, null, null);
        Habitacion aRetornar = null;
    ...
    private NodoAVLHabitaciones recuperarNodoAux(Comparable clave, NodoAVLHabitaciones nodo){
        NodoAVLHabitaciones aRetornar = new NodoAVLHabitaciones(null, null, null, null);
        if (nodo != null) {
            // Si el nodo es el elemento que busco, la busqueda fue un exito
